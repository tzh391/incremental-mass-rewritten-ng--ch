const MASS_DILATION = {
    ...
    massGain() {
		...
		if (player.gc.active || player.chal.active >= 21 || player.exotic.dark_run.active) x = GCeffect(x)
          tmp.dmOverflow = overflow(x,"e5e28",hasChargedElement(228)?1:hasChargedElement(196)?0.975:hasChargedElement(158)?0.95:hasChargedElement(21)?0.925:hasElement(196)?0.92:hasElement(158)?0.9:0.8).log(x);
