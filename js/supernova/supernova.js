...
function calcSupernova(dt, dt_offline) {
    ...
    if (su.fermions.unl) {
        ...
        else if (hasChargedElement(246)) for (let i = 2; i < 4; i++) for (let j = 5; j < 6; j++) if (j < FERMIONS.getUnlLength()) {
            su.supernova.fermions.tiers[i][j] = su.supernova.fermions.tiers[i][j]
            .max(tmp.supernova.fermions.tiers[i][j])   
        }
