const SUPERNOVA_GALAXY = {
	...
	effects:{
		...
		matter(){
			return (player.superGal.add(1).div(25000));
		},		
		em(){
	...
	galPow0_eff(){
		...
		if(hasElement(257))ss1p = E(1);
			...
			if(hasChargedElement(225))ss1p = ss1p.pow(0.8);
		}
		let ss2p=E(0.25);
		...
		if(hasChargedElement(225))ss2p = ss2p.pow(0.7);
	...
	galQkGain(){
		...
		if (player.exotic.times.gte(3))ret = ret.mul(player.exotic.times.add(1));
