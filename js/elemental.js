    ...
    upgs: [
		...
		},
		{
			...
			effect() {
				...
				if(hasChargedElement(232))x = x.add(10).log10().add(10).log10().pow(2).floor();
				return x
			},
			effDesc(x) {if(!hasChargedElement(232)) return format(x)+"x later";else if(hasChargedElement(232))return "当前战力"+format(x)
			},
			cdesc: `解锁鲨鱼战力，数值见下.`,
			ccost: E("e7735000"),									
		},
		...
		
		{
			...
			cost: E(3e10),
			galQk: true,
			cdesc: `鲨鱼碰到了 超级折算|元素需求 使从下一个开始的元素需要2种东西.`,
		    ccost: E("e7960000000"),		
		},
		{
			desc: `弱化第一个黑洞溢出.`,
			cost: E("1.5e704"),
			et: true,
			cdesc: `再这样下去质量就到不了eee154了，所以使坍缩星辰第三个效果^100(需:ee1.5555e124黑洞质量).`,
		    ccost: E("e8742500"),		
		},
		{
