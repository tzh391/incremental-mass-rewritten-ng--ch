const EXOTIC = {
    ...
    mils: [
        [E(1), `奇异重置时保持元素和10个超新星星系. 解锁新的量子里程碑.`],
        [E(2), `在奇异重置保持自动获得转生等级，荣耀，辉煌，名誉.`],
        [E(3), `奇异重置次数倍增量子次数和星系夸克获取.`],
        [E(4), `在奇异重置保持星系费米子完成次数.`],
        [E(5), `在奇异重置保持星系碎片.`],
    ...
    drGain(){
		...
		if(hasUpgrade('st',2))x = x.mul(player.stardust.add(10).log10().pow(0.5))
    ...
    dsEff(){
		...
		if(hasChargedElement(239))x.en = expMult(player.exotic.ds.add(2),20);
    ...
    abEff(){
		...
		if(hasChargedElement(238))x.csp = player.exotic.ab.add(1);
		return x;
    },
    axsVal(){
		...
		if(hasUpgrade('st',1))x = x.mul(player.stardust.add(1).log10().div(10000).add(1))
		return x;
    },
    axsRem(){
		let x = EXOTIC.axsVal();
