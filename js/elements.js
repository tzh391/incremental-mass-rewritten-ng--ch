function setupHTML() {
	...
	for (let x = 1; x <= UPGS.main.cols; x++) {
		...
		for (let y = 1; y <= UPGS.main[x].lens; y++) {
			...
			style="margin: 3px;" class="img_btn" id="main_upg_${x}_${y}" src="images/main_upg_${(x==7||x==6||y>15)?'placeholder':(id+y)}.png">`
		}
...
function updateUpperHTML() {
	...
	if (unl) tmp.el.sunAmt.setHTML(format(player.sunshard,0)+"<br>(+"+format(player.sunshardGain,0)+")")
}
	...
	if ((!tmp.supernova.reached || player.supernova.post_10) && tmp.tab != 5) {
		...
		if (tmp.tab == 0) {
			if (tmp.stab[0] == 0) {
				...
				tmp.el.masscholestasispower.setTxt(format(tmp.masscholestasispower))
				tmp.el.masscholestasis.setDisplay(player.mass.gte("eee100"))
			}
			if (tmp.stab[0] == 1) {
				updateBlackHoleHTML()
			}
